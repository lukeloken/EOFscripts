...
files <- list.files(file.path(path_to_data, "Approved_Site_Data",
                              "DiscoveryFarms"), full.names = TRUE)

files <- files[grepl('compiled', files) == FALSE]
files <- files[grepl('site.summary.report', files) == FALSE]


names <- list.files(file.path(path_to_data, "Approved_Site_Data",
                              "DiscoveryFarms"))
names <- names[grepl('compiled', names) == FALSE]
names <- names[grepl('site.summary.report', names) == FALSE]
...
# which(!names %in% site_table_i$Field_Name)

#Read in all files and compile. 
file_nu <- 24
data_list <- list()
for (file_nu in 1:length(files)){
  
  test_date_format1 <- test_date_format2 <- ".dplyr"
  ...
  test_date_format1 <- as.POSIXct(data_i$storm_start,  format = c("%m/%d/%y %H:%M"))
  test_date_format2 <- as.POSIXct(data_i$storm_start,  format = c("%m/%d/%Y %H:%M"))
  
  if (length(which(!is.na(test_date_format1))) > 0 & 
      median(year(test_date_format1), na.rm = T) > 2000){
    data_i <- data_i %>%
      mutate(across(c("storm_start", "storm_end", "sample_start", "sample_end"),
                    ~as.POSIXct(.x,
                                tz = "America/Chicago",
                                format = c("%m/%d/%y %H:%M"))))
  } else if (length(which(!is.na(test_date_format2))) > 0 &
             median(year(test_date_format2), na.rm = T) > 2000){
    data_i <- data_i %>%
      mutate(across(c("storm_start", "storm_end", "sample_start", "sample_end"),
                    ~as.POSIXct(.x,
                                tz = "America/Chicago",
  ...
  if (!'runoff_volume' %in% names(data_i)){
    ...
    } else if ('storm_runoff' %in% names(data_i)){
  ...
  if ("lab_no" %in% names(data_i)){
    data_i <- mutate(data_i, lab_no = as.character(lab_no))
  }
  
  if ("inst_q" %in% names(data_i)){
    data_i <- select(data_i, -inst_q)
  }
...
data_df <- data_list %>% 
  ...
  left_join(select(site_table_i, USGS_Station_Number, Field_Name, Area), 
            by = c("Field_Name")) %>%
  select(site, USGS_Station_Number, everything()) %>%
  mutate(site = gsub("\"", "", site))

head(data_df[,1:5])
identical(data_df[,1], data_df[,2])
...
total_dissolved_solids_names <- names(data_df)[grepl('total_dissolved_solid', 
                                                     names(data_df), ignore.case=T)]
total_suspended_solid_names <- names(data_df)[grepl('total_suspended_solid', 
                                                    names(data_df), ignore.case=T)]


# Combine names into a list. 
# The order of this list must match the preferred column names
...
print(data.frame(site_summary_report2[,c(1:4,9)]))
print(data.frame(site_summary_report2))


if (nrow(site_summary_report2) != length(names)){
  warning(paste0(toString(nrow(site_summary_report2)), " sites in dataset. Should be ", toString(length(names))))
  # print(merged_sites)
} else {
...
data_df_compiled <- data_df_new %>%
  select(-contains('used'), -contains('calculated'), -contains('new')) 
...
for (var_i in 1:length(concvars)) {
  ...
  if(calc_vars[var_i] %in% names(data_df_new) & 
    ...
    ggsave(file.path(path_to_results, 'Figures', 'ConcentrationTesting', 'DiscoveryFarms', paste0("DF_", concvars[var_i], '.png')),
           plot_list[[var_i]], height = round(length(names))/3, width = round(length(names))/2.4, units='in')
    ...
    ggsave(file.path(path_to_results, 'Figures', 'ConcentrationTesting', 'DiscoveryFarms', paste0('DF_', concvars[var_i], '_Diff.png')),
           plot_list2[[var_i]], height=4, width=12, units='in')
    ...
    ggsave(file.path(path_to_results, 'Figures', 'YieldTesting', 'DiscoveryFarms', paste0('DF_', yieldvars[var_i], '.png')),
           plot_list3[[var_i]], height = round(length(names))/3, width = round(length(names))/2.4, units='in')
    ...
    ggsave(file.path(path_to_results, 'Figures', 'ConcentrationTesting', 'DiscoveryFarms', paste0("DF_", loadvars[var_i], '.png')),
           plot_list4[[var_i]], height = round(length(names))/3, width = round(length(names))/2.4, units='in')
