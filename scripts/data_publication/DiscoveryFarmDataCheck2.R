...
# which(!names %in% site_table_i$FieldName)
...
data_df <- data_list %>% 
  ...
  left_join(select(site_table_i, USGSSiteNumber, FieldName, Area), 
            by = c("FieldName")) %>%
  select(site, USGSSiteNumber, everything()) %>%
...
for (var_i in 1:length(names_list)){
  ...
  if (length(names_list[[var_i]])>0){
    ...
    data_conc_i_unite[,1][which(data_conc_i_unite[,1] == 0)] <-NA
    data_conc_i_unite[,3][which(data_conc_i_unite[,3] == 0)] <-NA
    ...
    data_load_i_unite[,1][which(data_load_i_unite[,1] == 0)] <-NA
    data_load_i_unite[,2][which(data_load_i_unite[,2] == 0)] <-NA
...
for (var_i in 1:length(concvars)) {
  ...
  if(calc_vars[var_i] %in% names(data_df_withconc) & 
    ...
    ggsave(file.path(path_to_results, 'Figures', 'ConcentrationTesting', 'DiscoveryFarms', paste0("DF_", concvars[var_i], '.png')),
           plot_list[[var_i]], height=12, width=18, units='in')
    ...
    ggsave(file.path(path_to_results, 'Figures', 'YieldTesting', 'DiscoveryFarms', paste0('Yields_DF_', concvars[var_i], '.png')),
           plot_list3[[var_i]], height=10, width=15, units='in')
