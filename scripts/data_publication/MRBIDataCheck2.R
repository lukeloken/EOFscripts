...
summary(data_df$runoff_volume)
    ...
    data_df_new <- data_df
      ...
      data_conc_i_unite$used_conc_numeric[data_conc_i_unite$used_conc_numeric == 0] <- NA
    ...
    for (var_i in 1:length(concvars)) {
      ...
      ggsave(file.path(path_to_results, 'Figures', 'YieldTesting', 'MRBI', paste0('Yields_MRBI_', concvars[var_i], '.png')),
