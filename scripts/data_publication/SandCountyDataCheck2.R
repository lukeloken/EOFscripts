...
files <- files[-grepl('summary.report', files)]


names <- gsub("P:/0301/Approved_Site_Data/Sand County/", "", files)
...
data_df <- data_list %>% 
  bind_rows(.id = "Field_Name2") %>%
  mutate(Field_Name2 = gsub("WI-", "", Field_Name2)) %>%
  select(site, Field_Name = FieldName, Field_Name2, everything()) %>%
  left_join(select(site_table_i, Field_Name, Area), 
            by = c("Field_Name")) %>%
  select(-Field_Name2)
...
# Calculate loads and yields using runoff volume and concentration
# #########################################################
...
site_summary_report <- data_df_new %>%
  ...
  group_by(site, estimated, Field_Name) %>%
...
for (var_i in 1:length(concvars)) {
  ...
  if(calc_vars[var_i] %in% names(data_df_new) & 
    ...
    ggsave(file.path(path_to_results, 'Figures', 'YieldTesting', 'SandCounty', paste0('SC_', yieldvars[var_i], '.png')),
    ...
    ggsave(file.path(path_to_results, 'Figures', 'ConcentrationTesting', 'SandCounty', paste0("SC_", loadvars[var_i], '.png')),
           plot_list4[[var_i]], height = 4, width = 10, units='in')
