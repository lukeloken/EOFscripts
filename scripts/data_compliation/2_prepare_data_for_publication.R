...
common_vars_GLRI <- c("site", "Field_Name", "storm_start", "storm_end",
                 "peak_discharge", "runoff_volume")

approved_vars_GLRI <- c("discrete", "estimated", "exclude", 
                   "frozen", "unique_storm_number", "storm")


rain_vars_GLRI <- c("rain", "duration", "Ievent", 
               "I5", "I10", "I15", 
               "I30", "I60", "energy_m1", 
...
data_df_approved2 <- data_df_approved %>%
  select(intersect(names(data_df_approved),
                   all_of(c(common_vars_GLRI, approved_vars_GLRI, loadvars, 
                            concvars, flagvars, yieldvars)))) %>%
  filter(exclude == 0) %>%
  left_join(select(site_table, site_number = USGS_Station_Number, Field_Name),
...
data_df_approved3 <- data_df_approved2 %>%
  rename(Field_Name = site,
         site = site_number) %>%
  mutate(project = "GLRI") %>%
  select(site, Field_Name, project, discrete, estimated, frozen, storm, exclude,
         unique_storm_number, storm_start, storm_end, runoff_volume, peak_discharge, 
         everything()) 
...
#                    all_of(c(common_vars_GLRI, rain_vars_GLRI)))) %>%
