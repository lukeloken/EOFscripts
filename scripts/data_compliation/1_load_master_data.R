...
master_files <- list.files(file_in(file.path(path_to_data, 'field_analysis', 'approved_site_data', '0_masters')), 
                           full.names = TRUE, pattern = 'master', ignore.case = TRUE)

master_df <- data.frame(matrix(nrow=length(master_files), ncol=14))
