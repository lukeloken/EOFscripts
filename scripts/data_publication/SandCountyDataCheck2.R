...
sand_files <- sand_files[-grepl('summary.report', sand_files)]


sand_names <- gsub("P:/0301/Approved_Site_Data/Sand County/", "", sand_files)
...
for (file_nu in 1:length(sand_files)){
  ...
  names(data_list)[file_nu] <- sand_names[file_nu]
    ...
    data_df <- data_list %>% 
      bind_rows(.id = "FieldName2") %>%
      mutate(FieldName2 = gsub("WI-", "", FieldName2)) %>%
      select(site, FieldName, FieldName2, everything()) %>%
    ...
    site_summary_report <- data_df_new %>%
      ...
      group_by(site, estimated, FieldName) %>%
    ...
    for (var_i in 1:length(concvars)) {
      ...
      plot_list2[[var_i]] <- ggplot(data_df_withconc_i, 
                                    aes(y=diff, x=FieldName, 
                                        group=FieldName, fill=FieldName)) +
