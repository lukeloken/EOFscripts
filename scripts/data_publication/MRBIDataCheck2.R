#Check MRBI Farms concentration and load data
library(lubridate)
...
data_df <- data_list %>% 
  bind_rows(.id = "file_nu") %>%
  select(-file_nu) %>%
  mutate(site = as.character(site)) %>%
  mutate(site = gsub("'", "", site)) %>%
  select(site, Field_Name = FieldName, everything()) %>%
  left_join(select(site_table_i, Area, USGS_Station_Number), 
...
filter(data_df, runoff_volume<0.05)
    ...
    for (var_i in 1:length(concvars)) {
      ...
      if(calc_vars[var_i] %in% names(data_df_new) & 
        ...
        ggsave(file.path(path_to_results, 'Figures', 'ConcentrationTesting', 'MRBI', paste0("MRBI_", concvars[var_i], '.png')),
        ...
        ggsave(file.path(path_to_results, 'Figures', 'YieldTesting', 'MRBI', paste0('MRBI_', yieldvars[var_i], '.png')),
        ...
        ggsave(file.path(path_to_results, 'Figures', 'ConcentrationTesting', 'MRBI', paste0("MRBI_", loadvars[var_i], '.png')),
    ...
    data_df_approved2$storm[!grepl("Tile", data_df_approved2$Field_Name) &
